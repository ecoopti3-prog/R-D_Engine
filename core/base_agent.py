...
from core.schemas import AgentOutput, Finding, Idea, Kill, normalize_domain
...
def _normalize_domains_in_raw(raw: dict) -> dict:
    """Walk raw LLM output and normalize any 'domain' fields using normalize_domain().
    Returns a new dict — does not mutate the original."""
    import copy
    raw = copy.deepcopy(raw)
    for key in ("findings", "ideas"):
        for item in raw.get(key, []):
            if isinstance(item, dict) and "domain" in item:
                item["domain"] = normalize_domain(item["domain"])
    return raw
...
class BaseAgent:
    ...
    def run(self, context: dict, cycle_id: str) -> AgentOutput:
        ...
        try:
            ...
            err_str = str(e)
            # ── Domain normalization rescue ───────────────────────────────────
            # If the only failure is an invalid 'domain' value, normalize it
            # and retry rather than dropping the entire agent output.
            if "domain" in err_str and "literal_error" in err_str:
                logger.warning(
                    f"[{self.AGENT_NAME}] Domain validation error — attempting auto-normalize: {err_str[:200]}"
                )
                try:
                    raw = _normalize_domains_in_raw(raw)
                    output = self.parse_output(raw, cycle_id)
                    output.llm_used    = llm_used
                    output.tokens_used = total_tokens
                    output.duration_ms = duration_ms
                    output.status      = "done"
                    logger.info(
                        f"[{self.AGENT_NAME}] Domain-normalized rescue OK — "
                        f"{len(output.findings)} findings, {len(output.ideas)} ideas"
                    )
                    return output
                except Exception as e2:
                    logger.error(f"[{self.AGENT_NAME}] Rescue also failed: {e2}")
