...
def _extract_venue(url: str) -> str:
    ...
    if "patents.google.com" in url or "patents.justia.com" in url: return "patent"
    if "lens.org" in url:                                           return "patent"
    if "semanticscholar.org" in url:                                return "semantic_scholar"
    if "core.ac.uk" in url:                                         return "core"
    if "huggingface.co" in url:                                     return "huggingface"
    if "github.com" in url:                                         return "github"
    if "linkedin.com" in url or "jobs" in url:                      return "job_posting"
    if "greenhouse.io" in url:                                      return "job_posting"
    if "lever.co" in url:                                           return "job_posting"
    if "myworkdayjobs.com" in url:                                  return "job_posting"
    ...
    if "sec.gov" in url or "edgar" in url:                          return "sec_edgar"
    if "darpa.mil" in url or "sbir.gov" in url:                     return "darpa_baa"
    if "grants.gov" in url:                                         return "darpa_baa"
    if "ntrs.nasa.gov" in url or "technology.nasa.gov" in url:      return "nasa"
    if "osti.gov" in url:                                           return "osti_doe"
    if "doi.org" in url:                                            return "crossref"
    if "crossref.org" in url:                                       return "crossref"
    if "openalex.org" in url:                                       return "openalex"
    if "spectrum.ieee.org" in url:                                  return "ieee_spectrum"
    if "eetimes.com" in url:                                        return "ee_times"
    if "semianalysis.com" in url:                                   return "semianalysis"
    if "tomshardware.com" in url:                                   return "tomshardware"
    if "semiwiki.com" in url:                                       return "semiwiki"
    if "theregister.com" in url:                                    return "theregister"
    if "arstechnica.com" in url:                                    return "arstechnica"
    if "therobotreport.com" in url:                                 return "robot_report"
    if "datacenterknowledge.com" in url:                            return "datacenter_knowledge"
    if "datacenterdynamics.com" in url:                             return "dcd"
    return "other"
