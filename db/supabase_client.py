        ...
        for row in res.data:
            try:
                ...
                idea = Idea(
                    id=row["id"],
                    title=row.get("title", ""),
                    domain=row.get("domain", "hardware"),
...
def save_search_strategy(strategy: dict) -> bool:
    ...
    try:
        # Try update first (row already created by save_daily_summary)
        res = db.table("daily_summaries").update({
...
def load_search_strategy() -> dict:
    ...
    Falls back to seed.json defaults on day 1 (empty DB) so PhysicsLimitMapper
    always has meaningful context even on first run.
    """
    db = get_client()
    try:
        res = (
    ...
    # ── Day-1 / cold-start fallback: seed.json provides the initial strategy ──
    # Without this, PhysicsLimitMapper runs with empty context on first execution.
    try:
        import json
