...
class IntelligenceResearcher(BaseAgent):
    ...
    def build_user_prompt(self, context: dict) -> str:
        ...
        memory_text = ""
        if kill_patterns:
            memory_text += f"\nDEAD ENDS (directions we already know fail — do NOT generate findings/ideas here): {', '.join(kill_patterns)}"
        if killed_titles:
            memory_text += "\nPREVIOUSLY KILLED IDEAS (do not regenerate):\n" + "\n".join(f"  - {t}" for t in killed_titles[:10])
        if diamond_titles:
            memory_text += "\nSUCCESSFUL DIRECTIONS (go deeper, find more company signals here):\n" + "\n".join(f"  + {t}" for t in diamond_titles[:5])

        job_text = ""
        if job_signals:
            job_text = "\nJOB POSTINGS RETRIEVED (use these as primary signal):\n"
            for j in job_signals[:15]:
                job_text += f"  [{j.get('company','')}] {j.get('title','')} — {j.get('description','')[:200]}\n"
        else:
            job_text = "\nNo job postings retrieved — use your knowledge of recent public hiring announcements."
...
ROBOTICS / INDUSTRIAL:
- Wiring harness failures in robotic arms (Joule heating, arc flash, connector degradation)
- Gear and joint fatigue in industrial robots (wear, vibration, S-N curves)
- Motor/servo thermal derating and bearing fatigue at high duty cycles
- Vibration-induced failures in precision motion systems

LIQUID COOLING / DATA CENTER:
- Coolant leaks and corrosion in server liquid cooling loops
- Cavitation in cooling pumps at high flow rates
- Pressure drop and thermal capacity limits in cold plates
- Galvanic corrosion between dissimilar metals in cooling loops

IMPORTANT: domain field must be one of: thermal, power, data_movement, hardware, pdn, cross_domain,
packaging, bandwidth, memory, interconnect, networking, software, compute_scheduling,
hardware_utilization, compute_resource_management, distributed_systems, edge, inference, compilation, storage.
Never use any other value for domain.
